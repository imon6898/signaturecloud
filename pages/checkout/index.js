...
const Checkout = () => {
  ...
  const currencySymbol = settings.settingsData.currency.symbol || "Tk";
  ...
  const setDeliveryArea = (event) => {
    const area = event.target.value; // Get selected radio button value
    setSelectedArea(area); // Update state

    console.log("Selected area: ", area);

    const areaInfo = shippingChargeInfo?.area.find((item) => item._id === area);
  ...
  //               ))
  //             ) : (
  //               <option disabled>{t("No areas available")}</option>
  //             )}
  //           </select>
  //         </div> */}
  //         <div>
            ...
            shippingChargeInfo.area.map((ct, idx) => (
              <div key={idx} className="form-check">
                <input
                  ...
                  className="form-check-input"
                  onChange={setDeliveryArea}
                  checked={selectedArea === ct._id} // Ensures only one is selected
                />
                <label htmlFor={`area-${ct._id}`} className="form-check-label">
                  {ct.name}
                </label>
              </div>
            ))
