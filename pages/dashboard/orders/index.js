...
const OrderPage = () => {
  ...
  const handlePaymentStatus = async (e, orderStatus, id) => {
    ...
    const url = `${process.env.NEXT_PUBLIC_URL}/api/order/${id}?payment_status=${e.target.value}&order_status=${orderStatus}&action=payment`;
    await postData(url)
      .then((data) =>
        data.success
  ...
  const handleOrderStatus = async (e, paymentStatus, id) => {
    const url = `${process.env.NEXT_PUBLIC_URL}/api/order/${id}?payment_status=${paymentStatus}&order_status=${e.target.value}&action=order`;
