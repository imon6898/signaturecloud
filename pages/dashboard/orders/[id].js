...
import { updateData, postData } from "~/lib/clientFunctions";
...
const ViewOrder = () => {
  ...
  const updateCourierSelection = async () => {
    if (!router.query.id) return;

    const url = `/api/order/${router.query.id}`;
    const response = await postData(url, { courier: selectedCourier });
    console.log("response: ", response);
    console.log("selectedCourier: ", selectedCourier);

    if (response.success) {
      toast.success("Courier updated successfully!");
      mutate(url); // Refresh order data after update
    } else {
      toast.error("Something went wrong while updating courier!");
    }
  };
              ...
              <div>
                <div className={`${classes.order_details} row`}>
                  ...
                  <div className="col-md-6">
                    ...
                    <p>
                      {t("house")} :{" "}
                      <span>{orderData.shippingInfo?.house}</span>
                    </p>
                  </div>
                  <div className="col-md-6">
                    <h6>Send Courier :</h6>
                    {/* Courier Selection Buttons */}
                    ...
                    <button
                      onClick={updateCourierSelection}
