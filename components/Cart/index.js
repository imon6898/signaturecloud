...
const CartPage = () => {
  ...
  const getTotalVat = decimalBalance(
    cart.items.reduce(
      (accumulator, item) =>
        accumulator + checkPercentage(item.qty * item.price),
      0
    )
  );
  ...
  const finalPrice = getTotalPrice - discountPrice;
      ...
      <div className={classes.card_container}>
        ...
        {/* {getTotalVat > 0 && (
          <div className={classes.card}>
            <p>{t("vat")}</p>
            <b>
              {settings.settingsData.currency.symbol ? settings.settingsData.currency.symbol : "Tk"}
